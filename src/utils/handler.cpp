...
cv::Mat StereoVideoHandler::GetNewFrame(){
 ...
 // for (int i = 0; i < 5; ++i)
  ...
  //temporarly for the kind of videos testing on
  if (rhs.size() != cv::Size(720, 576)){
	  cv::Mat resized;
	  cv::resize(to_return, resized, cv::Size(720, 576));
	  to_return = resized;
  }
...
void VideoHandler::SaveFrame(const cv::Mat image){
  ...
  // open the writer to create the processed video
	int fps = 12; //fps of output video
    writer_.open(output_url_,CV_FOURCC('M','J','P','G'), fps, 
